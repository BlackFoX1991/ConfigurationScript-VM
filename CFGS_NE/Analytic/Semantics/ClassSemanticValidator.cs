...
namespace CFGS_VM.Analytic.Semantics
{
    internal sealed class ClassSemanticValidator
    {
        ...
        private static string MethodArityShape(FuncDeclStmt method)
            => MethodArityShape(method.Parameters.Count, method.MinArgs, !string.IsNullOrWhiteSpace(method.RestParameter));

        private static string MethodArityShape(InterfaceMethodDecl method)
            => MethodArityShape(method.Parameters.Count, method.MinArgs, !string.IsNullOrWhiteSpace(method.RestParameter));
